...
function verificarLogin(lugar) {
	...
	$.ajax({
		type: 'POST'
		, url: "http://192.168.1.102/Servidor/Usuario.asmx/verificarLogin"
		, contentType: 'application/json; charset=utf-8'
		, dataType: 'json'
		...
		, success: function (data, status) {						
			...
			if(retorno=="OK" && lugar=="principal"){
				return;		
			}else if(retorno=="OK" && lugar!="principal"){
				window.location = "principal.html";
				return;
			}else if(retorno!="OK" && (lugar=="index" || lugar=="login")){
				return;
			}else{
...
function logout(){
	...
	$.ajax({
        type: 'POST'
        , url: "http://192.168.1.102/Servidor/Usuario.asmx/logout"
        , contentType: 'application/json; charset=utf-8'
        , dataType: 'json'
...
function recuperarSenha(){
	...
	$.ajax({
        type: 'POST'
        , url: "http://192.168.1.102/Servidor/Usuario.asmx/recuperarSenha"
        , contentType: 'application/json; charset=utf-8'
        , dataType: 'json'
...
function cadastrarUsuario() {
	...
	if(nome!='' && email!='' && senha!=''){
		...
		if(senha == confirmar){		
			$.ajax({
                type: 'POST'
                , url: "http://192.168.1.102/Servidor/Usuario.asmx/cadastrarUsuario"
				, crossDomain:true
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
                ...
                , success: function (data, status){                    
					...
					if(retorno.erro != "Erro de Usuário"){
						...
						localNotification('Cadastro realizado!','Usuário cadastrado com sucesso');
...
function atualizarSenhaUsuario() {
	...
	if(email.trim()!='' && senha.trim()!='' && novaSenha.trim()!=''){
		...
		if(novaSenha == confirmar){		
			$.ajax({
                type: 'POST'
                , url: "http://192.168.1.102/Servidor/Usuario.asmx/atualizarSenhaUsuario"
				, crossDomain:true
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
...
//__________________ LOCAL NOTIFICATION ___________________//
window.localNotification = function(titulo,menssagem)
{
